...
async function mintNFT(user) {
  ...
  try {
    ...
    return { tx1: operation.id, tx2: operation2.id }; // Return the transaction ID for tracking
  } catch (error) {
...
async function checkEvolutions() {
  const usersWithNFT = await User.find({ nftMinted: true });
  const evolution = await Evolution.find({ userId: usersWithNFT.id });
  const now = new Date();
